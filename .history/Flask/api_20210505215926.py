...
def add_artwork(user_id):
    ...
    try:
        ...
        try:
            ...
            sql_create_block = ''' INSERT INTO block(index, previous_hash, timestamp, hash, nonce, id_artwork, id_user_creator, id_user_owner) 
                                VALUES (:index, :previous_hash, :timestamp, :hash, :nonce, :id_artwork, :id_user_creator, :id_user_owner) '''
            data_block = {
                'index': block.index,
            ...
            cursor.execute(sql_create_block, data)
            connect.commit()
