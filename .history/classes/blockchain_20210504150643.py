...
class Blockchain:
    ...
    def validate_block(block):
        print(block)
        hash = hashlib.sha256(str({ 'index': block.index, 'timestamp': block.timestamp, 'nonce': block.nonce, 'previous_hash': block.previous_hash, 'data': block.data}).encode('utf-8')).hexdigest()
        print(hash)
        print(block.hash)
